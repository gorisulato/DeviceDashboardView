var Dashboard2 = new function (param) {
    ...
    this.init = function () {
        ...
        channel.bind('my-event', function (data) {
            ...
            //console.log(data.message[0].DeviceID)
            var deviceid = data.message[0].DeviceID

            if (deviceid == $("#DeviceIDDashboard").val()) {

                if (document.getElementById("chart-detail" + data.message[0].SensorName) == null) {
                    a = ' <div class="chart-container-detail animated fadeInRight">'
                           // + ' <h1 id="warning-chart' + data.message[0].DeviceID + '" class="warning-blink-detail ' + warning + ' ">Warning!!!</h1>'
                            + '<h1 class="title-chart">' + data.message[0].SensorName + '</h1> ' +
                            '<div id="chart-detail' + data.message[0].SensorName + '" > ' +
                    ...
                    var color = [""]
                    Dashboard2.GenerateChartDetail(data.message[0].SensorName, data.message[0].lower, data.message[0].upper, data.message[0].Value)
                }
                else {
                    Dashboard2.UpdateChart(data.message[0].SensorName, data.message[0].lower, data.message[0].upper, data.message[0].Value)
                }
            }
        });
